...
# These can be overridden in CI without changing the source code.
TEST_PASSWORD = os.getenv(
    "TEST_PASSWORD",
    "test-password-for-integration-tests",
)

WRONG_PASSWORD = os.getenv(
    "WRONG_TEST_PASSWORD",
    "wrong-test-password",
)
...
def register_and_login(
    ...
    """Register a test user and return authenticated headers."""
...
class TestDeployments:
    ...
    def test_create_and_list_deployment(self, client):
        ...
        resp = client.post(
            ...
            json={
                "commit_sha": "abc123",
                "service_name": "privacy-aware-app",
                "environment": "production",
            },
            headers=headers,
        )
    ...
    def test_approve_requires_admin_or_reviewer_role(
        self,
        client,
    ):

        dev_headers = register_and_login(
            client,
            "ivan",
            "developer",
        )
        ...
        resp = client.post(
            f"/api/deployments/{dep['run_id']}/approve",
            headers=dev_headers,
        )
    ...
    def test_get_nonexistent_deployment_404s(
        self,
        client,
    ):

        resp = client.get(
            "/api/deployments/run-doesnotexist"
        )
...
class TestAuditTrail:
    ...
    def test_deployment_creation_writes_audit_event(
        self,
        client,
    ):
        ...
        assert any(
            e["deployment_id"] == dep["id"]
            and e["event_type"] == "deployment_created"
            for e in events
        )

    def test_audit_chain_hashes_link(
        self,
        client,
    ):
        ...
        for prev, curr in zip(
            ...
            assert (
                curr["prev_hash"]
                == prev["event_hash"]
            ), "audit chain is not properly linked"
