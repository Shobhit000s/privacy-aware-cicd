...
CERT_RE = re.compile(r"-----BEGIN (CERTIFICATE|PRIVATE KEY|RSA PRIVATE KEY|EC PRIVATE KEY|PUBLIC KEY)-----")
PASSWORD_ASSIGN_RE = re.compile(r"""(?i)\b(password|passwd|pwd)\s*[:=]\s*['"][^'"\s]{4,}['"]""")
CREDIT_CARD_RE = re.compile(r"\b(?:\d[ -]*?){13,19}\b")
IP_ADDR_RE = re.compile(r"\b(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\b")
...
DEFAULT_IGNORE_GLOBS = ["*.lock", "*.min.js", "*package-lock.json*", "*synthetic_*.csv"]
...
def pan_format_valid(value: str) -> bool:
    # PAN structure: 5 letters, 4 digits, 1 letter; 4th letter encodes holder type
    return bool(re.fullmatch(r"[A-Z]{3}[ABCFGHLJPT][A-Z]\d{4}[A-Z]", value))
...
def scan_file(path: str) -> list[Finding]:
    ...
    for lineno, line in enumerate(lines, start=1):
        for m in EMAIL_RE.finditer(line):
            findings.append(Finding("email", SEVERITY["email"], path, lineno, redact(m.group())))

        for m in PHONE_IN_RE.finditer(line):
            findings.append(Finding("phone_number", SEVERITY["phone_number"], path, lineno, redact(m.group())))

        for m in AADHAAR_RE.finditer(line):
            digits = re.sub(r"\s", "", m.group())
            if len(digits) == 12 and verhoeff_checksum_valid(digits):
                findings.append(Finding("aadhaar", SEVERITY["aadhaar"], path, lineno, redact(digits)))

        for m in PAN_RE.finditer(line):
            if pan_format_valid(m.group()):
                findings.append(Finding("pan", SEVERITY["pan"], path, lineno, redact(m.group())))

        for m in JWT_RE.finditer(line):
            findings.append(Finding("jwt", SEVERITY["jwt"], path, lineno, redact(m.group())))

        for m in CERT_RE.finditer(line):
            findings.append(Finding("certificate_or_key", SEVERITY["certificate_or_key"], path, lineno, m.group()))

        for m in PASSWORD_ASSIGN_RE.finditer(line):
            findings.append(Finding("password_assignment", SEVERITY["password_assignment"], path, lineno, redact(m.group())))

        for m in CREDIT_CARD_RE.finditer(line):
            digits = re.sub(r"[ -]", "", m.group())
            if luhn_valid(digits):
                findings.append(Finding("credit_card", SEVERITY["credit_card"], path, lineno, redact(digits)))
...
def scan_path(root: str, ignore_dirs=None, ignore_globs=None) -> list[Finding]:
...
def main():
    ...
    ap.add_argument("--fail-on", choices=["LOW", "MEDIUM", "HIGH", "CRITICAL", "none"], default="HIGH")
    args = ap.parse_args()

    if args.demo:
        os.makedirs("/tmp/privacy_scan_demo", exist_ok=True)
        with open("/tmp/privacy_scan_demo/sample.py", "w") as f:
            f.write(
        ...
        findings = scan_path("/tmp/privacy_scan_demo")
    else:
    ...
    if args.format == "json":
        ...
        lines = [f"Privacy scan: {summary['total']} finding(s) across {len(set(f.file for f in findings))} file(s)"]
        for f in findings:
            lines.append(f"  [{f.severity:8s}] {f.rule:20s} {f.file}:{f.line}  {f.redacted_match}")
        lines.append("")
    ...
    if args.fail_on != "none":
        ...
        if blocking:
            print(f"\n❌ {len(blocking)} finding(s) at or above {args.fail_on} severity. Failing.", file=sys.stderr)
            sys.exit(1)
